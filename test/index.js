...
test('Invalid construction of GithubWebHook handler', function(t) {
	...
	t.throws(GithubWebHook.bind(null, ''), /must provide an options object/,
		'throws if option is not an object');
...
test('GithubWebHook handler is an EventEmitter', function(t) {
	...
	t.throws(handler.emit.bind(handler, 'error', new Error('error')), /error/,
		'works as a EventEmitter');
});
...
test('Ignore unmatched request method', function(t) {
	...
	request(app)
		...
		.end(function(err, res) {
			t.deepEqual(res.body, {
				message: 'Here'
			}, 'ignore unmatched request method');
		});
});
...
test('Invalid request meta', function(t) {
	...
	request(app)
		...
		.end(function(err, res) {
			t.deepEqual(res.body, {
				error: 'No event found in the request'
			}, 'request should have event');
		});
...
test('Accept a valid request with form-urlencoded data', function(t) {
	t.plan(8);
	/**
	 * Create mock express app
	 */
	...
	request(app)
		...
		.set('X-Hub-Signature', signData('secret', 'payload=' + json))
		.expect('Content-Type', /json/)
		.expect(200)
		.end(function(err, res) {
			t.deepEqual(res.body, {
	...
	webhookHandler.on('repo', function(event, data) {
		t.equal(event, 'issues', 'receive a issues event on event \'repo\'');
		t.deepEqual(data, data, 'receive correct data on event \'repo\'');
	});

	webhookHandler.on('issues', function(repo, data) {
		t.equal(repo, 'repo', 'receive a event for repo on event \'issues\'');
		t.deepEqual(data, data, 'receive correct data on event \'issues\'');
	});

	webhookHandler.on('*', function(event, repo, data) {
		t.equal(event, 'issues', 'receive a issues event on event \'*\'');
		t.equal(repo, 'repo', 'receive a event for repo on event \'*\'');
		t.deepEqual(data, data, 'receive correct data on event \'*\'');
	});
});
